...
while IFS= read -r -d '' f; do
  name="$(basename "$f")"
  case "$name" in
    package-lock.json|README.md|CHANGELOG.md|CLAUDE.md|LICENSE) continue ;;
  esac
  config_files+=("$name")
...
# AGENTS.md (multi-tool AI agent instructions — key config for this project)
[[ -f "$ROOT/AGENTS.md" ]] && config_files+=("AGENTS.md")
...
# .github/plugin/ (Copilot CLI marketplace manifests)
if [[ -d "$ROOT/.github/plugin" ]]; then
  while IFS= read -r -d '' f; do
    relpath="${f#$ROOT/}"
    config_files+=("$relpath")
  done < <(find "$ROOT/.github/plugin" -maxdepth 1 -type f -print0 2>/dev/null | sort -z)
fi

# plugins/ manifest and skill files
if [[ -d "$ROOT/plugins" ]]; then
  while IFS= read -r -d '' f; do
    relpath="${f#$ROOT/}"
    config_files+=("$relpath")
  done < <(find "$ROOT/plugins" -maxdepth 4 \( -name '*.json' -o -name 'SKILL.md' \) -type f -print0 2>/dev/null | sort -z)
fi
